...
class BackupTests(ScenarioTest, unittest.TestCase):
    ...
    def test_backup_wl_container(self, container_name1='shrac3', container_name2='shrac2', resource_group='shracrg', vault_name='shracsql'):
        ...
        container_json = self.cmd('backup azure-wl container show -n {name1} -v {vault} -g {rg}', checks=[
        ...
        self.cmd('backup azure-wl container show -n {container_name} -v {vault} -g {rg}', checks=[
            self.check('properties.friendlyName', '{name1}'),
            self.check('properties.healthStatus', 'Healthy'),
            self.check('properties.registrationStatus', 'Registered'),
            self.check('name', '{container_name}'),
        ...
        self.cmd('backup azure-wl container list -v {vault} -g {rg}', checks=[
            # self.check("length(@)", 2),
    ...
    def test_backup_wl_item(self, container_name1='shrac3', container_name2='shrac2', resource_group='shracrg', vault_name='shracsql', policy_name='HourlyLogBackup'):
        ...
        self.kwargs['container1'] = self.cmd('backup azure-wl container show -n {name1} -v {vault} -g {rg} --query name').get_output_in_json()
        self.kwargs['container2'] = self.cmd('backup azure-wl container show -n {name2} -v {vault} -g {rg} --query name').get_output_in_json()

        item1_json = self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1} -n {item1} -wt MSSQL', checks=[
        ...
        self.kwargs['container1_fullname'] = self.cmd('backup azure-wl container show -n {name1} -v {vault} -g {rg} --query name').get_output_in_json()

        self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1_fullname} -n {item1} -wt MSSQL', checks=[
            self.check('properties.friendlyName', '{item1}'),
            self.check('properties.protectedItemHealthStatus', 'NotReachable'),
            self.check('properties.protectionState', 'IRPending'),
            self.check('properties.protectionStatus', 'Healthy'),
        ...
        self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1_fullname} -n {item1_fullname} -wt MSSQL', checks=[
            self.check('properties.friendlyName', '{item1}'),
            self.check('properties.protectedItemHealthStatus', 'NotReachable'),
            self.check('properties.protectionState', 'IRPending'),
            self.check('properties.protectionStatus', 'Healthy'),
        ...
        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container1} -wt MSSQL', checks=[
            # self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item1}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container1_fullname} -wt MSSQL', checks=[
            # self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item1}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container2} -wt MSSQL', checks=[
            self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item2}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault} -wt MSSQL', checks=[
            # self.check("length(@)", 2),
            self.check("length([?properties.friendlyName == '{item1}'])", 1),
            self.check("length([?properties.friendlyName == '{item2}'])", 1)
        ])

        # self.cmd('backup azure-wl item set-policy -g {rg} -v {vault} -c {container1} -n {item1} -p {policy}', checks=[
        ...
        item1_json = self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1} -n {item1} -wt MSSQL').get_output_in_json()
        self.assertIn(policy_name.lower(), item1_json['properties']['policyId'].lower())
    ...
    def test_backup_wl_rp(self, container_name='VMAppContainer;compute;shracrg;shrac4', resource_group='shracrg', vault_name='shracsql', item_name='msdb'):
        ...
        self.cmd('backup azure-wl recoverypoint list -g {rg} -v {vault} -c {name} -i {item} -wt MSSQL --query [].name', checks=[
            self.check("length(@)", 2)
        ]).get_output_in_json()

        rp1_json = self.cmd('backup azure-wl recoverypoint logchain show -g {rg} -v {vault} -c {name} -i {item} -wt MSSQL', checks=[
            self.check('[].resourceGroup', '[\'{rg}\']')
        ]).get_output_in_json()
        self.assertIn(vault_name.lower(), rp1_json[0]['id'].lower())
        self.assertIn(container_name.lower(), rp1_json[0]['id'].lower())

        rp2_json = self.cmd('backup azure-wl recoverypoint logchain show -g {rg} -v {vault} -c {name} -i {item} -wt MSSQL', checks=[
            self.check('[].resourceGroup', '[\'{rg}\']')
        ]).get_output_in_json()
    ...
    def test_backup_protectable_item(self, container_name1='shrac3', container_name2='shrac2', resource_group='shracrg', vault_name='shracsql', policy_name='HourlyLogBackup'):
        ...
        # self.kwargs['container1'] = self.cmd('backup azure-wl container show -n {name1} -v {vault} -g {rg} --query properties.friendlyName').get_output_in_json()
        # self.kwargs['container2'] = self.cmd('backup azure-wl container show -n {name2} -v {vault} -g {rg} --query properties.friendlyName').get_output_in_json()

        self.kwargs['name'] = self.cmd('backup azure-wl protectable-item list -g {rg} -v {vault} --query [0].name').get_output_in_json()

        item1_json = self.cmd('backup azure-wl protectable-item show -g {rg} -v {vault} -n {name}', checks=[
        ...
        self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1_fullname} -n {item1}', checks=[
            self.check('properties.friendlyName', '{item1}'),
            self.check('properties.protectedItemHealthStatus', 'NotReachable'),
            self.check('properties.protectionState', 'IRPending'),
            self.check('properties.protectionStatus', 'Healthy'),
        ...
        self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1_fullname} -n {item1_fullname}', checks=[
            self.check('properties.friendlyName', '{item1}'),
            self.check('properties.protectedItemHealthStatus', 'NotReachable'),
            self.check('properties.protectionState', 'IRPending'),
            self.check('properties.protectionStatus', 'Healthy'),
        ...
        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container1}', checks=[
            #self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item1}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container1_fullname}', checks=[
            #self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item1}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault} -c {container2}', checks=[
            self.check("length(@)", 1),
            self.check("length([?properties.friendlyName == '{item2}'])", 1)
        ])

        self.cmd('backup azure-wl item list -g {rg} -v {vault}', checks=[
            #self.check("length(@)", 2),
            self.check("length([?properties.friendlyName == '{item1}'])", 1),
            self.check("length([?properties.friendlyName == '{item2}'])", 1)
        ])

        item1_json = self.cmd('backup azure-wl item show -g {rg} -v {vault} -c {container1} -n {item1}').get_output_in_json()
        self.assertIn(policy_name.lower(), item1_json['properties']['policyId'].lower())