...
def get_default_policy_for_vm(client, vault):
    ...
    return protection_policies_client.get(rs_vault.name, resource_group, 'DefaultPolicy')
...
def update_policy_for_item(client, backup_item, policy):
    ...
    # Update policy
    result = protected_items_client.create_or_update(
        vault_name, resource_group, "Azure", container_uri, item_uri, vm_item, raw=True)
...
def backup_now(client, backup_item, retain_until):
    ...
    # Trigger backup
    result = backups_client.trigger(vault_name, resource_group, "Azure", container_uri, item_uri, trigger_backup_request, raw=True)
    return _track_backup_operation(result, vault_name, resource_group)
...
def show_recovery_point(client, id, backup_item):
    ...
    return recovery_point_client.get(vault_name, resource_group, "Azure", container_uri, item_uri, id)
...
def list_recovery_points(client, backup_item, start_date=None, end_date=None):
    ...
    # Get recovery points
    recovery_points = recovery_point_client.list(vault_name, resource_group, "Azure", container_uri, item_uri, filter_string)
...
def restore_disks(client, recovery_point, destination_storage_account, destination_storage_account_resource_group):
    ...
    for item in paged_items:
        if container_name.lower() in item.properties.container_name.lower():
            if item.properties.friendly_name.lower() == item_name.lower():
                filtered_items.append(item)
    ...
    # Trigger restore
    result = restore_client.trigger(vault_name, resource_group, "Azure", container_uri, item_uri, _recovery_point_id, trigger_restore_request, raw=True)
    return _track_backup_operation(result, vault_name, resource_group)
...
def disable_protection(client, backup_item, delete_backup_data=False, yes=False):
    ...
    if delete_backup_data:
        result = protected_items_client.delete(vault_name, resource_group, "Azure", container_uri, item_uri, raw=True)
        return _track_backup_operation(result, vault_name, resource_group)
    else:
        vm_item_properties = _get_vm_item_properties_from_vm_id(item.properties.virtual_machine_id)
...
def _track_backup_operation(result, vault_name, resource_group):
    ...
    if operation_status.properties is not None:
...
def _job_in_progress(job_status):
    if job_status == "InProgress" or job_status == "Cancelling":
        return True
    return False
